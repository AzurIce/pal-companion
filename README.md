...
2. 进入存档后，按 `F7`（或点击网页页脚状态栏的“刷新”按钮）显式触发一次同步。
   Palws 会向游戏服务器请求全部 32 页数据，等待复制结算后生成一份全量快照发送给网页，通常需要约 10 秒。
3. 网页收到快照后执行全量替换：本次同步到的帕鲁会重建“游戏同步”列表，手工添加的条目保留。
4. 同步期间页脚会显示“已排队 / 请求中 / 采集中 / 已同步”；出错时保留上一份成功快照，可再次刷新重试。
5. 页脚最右侧的“日志”按钮可展开同步控制台；日志区域支持滚动、文本选择和一键复制诊断内容。

生产版没有自动同步：打开帕鲁终端、切换地图或新建对象都不会触发请求，只有 `F7` 或网页“刷新”按钮会触发。两次同步之间默认有约 15 秒冷却。网页刷新与 `F7` 使用同一个 Lua 快照请求入口。

据点筛选会按游戏中的据点容器分组；据点较多时筛选栏可横向滚动，不会把按钮文字压成竖排。同步快照采用全量替换语义，但手工登记的条目会保留。

如果网页一直显示“未连接”，检查以下位置：

- Mod 文件是否位于上面的准确目录，而不是多套了一层文件夹；
- `Palworld\Mods\NativeMods\UE4SS\UE4SS.log` 中是否有 `[Palws] require 'palws': ok=true`、`[Palws] start_server: ok=true` 和 `F7 = request snapshot`；
- 如果端口已被其他进程占用，日志会明确显示 `bind failed on 127.0.0.1:32123`；关闭占用端口的程序后再启动游戏；
