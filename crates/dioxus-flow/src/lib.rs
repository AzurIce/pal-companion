...
//!
//! Forked from xiv-companion's `crates/dioxus-flow` with additions:
//! - `FlowEdge::dashed` / `FlowEdge::emphasis` rendered as CSS classes
//!   (`flow-edge--dashed` / `flow-edge--highlight` / `flow-edge--dim`) so
//!   callers can style edge kinds and hover emphasis from their own CSS.
//! - `FlowCanvas::animate` enables a CSS transition on viewport movement
//!   (use it for programmatic fit/reset, keep it off while dragging).
